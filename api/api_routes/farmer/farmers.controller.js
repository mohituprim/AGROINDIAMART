const farmersRepo = require('../../api_utils/farmer/farmersRepository')
...
class FarmersController {
    ...
    saveSellOrder(req, res) {
        farmersRepo.saveSellOrder(req.body, (err, order) => {
            if (err) {
                console.log('*** order error: ');
                res.json({status: false, error: 'Insert failed', order: null});
            } else {
                console.log('*** order ok');
                this.getSellOrders(req, res);
            }
        });
    }
    getSellOrders(req, res) {
        console.log('*** getSellOrders');
        farmersRepo.getSellOrders((err, data) => {
            if (err) {
                console.log('*** getSellOrders error: ');
                res.json({status: false, error: 'failed', orders: null});
            } else {
                console.log('*** getSellOrders ok');
                res.json({ status: true, error: null, orders: data });
            }
        });
    }
